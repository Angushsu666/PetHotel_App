...
class FilterConditionsPage extends StatefulWidget {
  ...
  const FilterConditionsPage({super.key, required this.sortedShops});
    ...
    setState(() {
      filteredShops = widget.sortedShops.where((shop) {
        ...
        bool roomTypeMatch = roomTypeSelections.entries.every((entry) {
          if (!entry.value) {
            return true; // 如果用户没有选择这个房型，我们认为这个条件是匹配的
          }
          // 检查商店是否提供这个房型，并且至少有一个类别的价格不为0
          return shop.roomCollection[entry.key] != null &&
              shop.roomCollection[entry.key]!.values.any((price) => price != 0);
        });
  ...
  Widget buildFilterSection(String title, Map<String, bool> selections) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(title),
          const SizedBox(height: 10), // 添加一点垂直间距
          Row(
