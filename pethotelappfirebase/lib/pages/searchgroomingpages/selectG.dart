...
import 'selectGrooming.dart';
import '../messageBox.dart'; // 注意这里导入 MessagePage 的路径
import '../../providers/user_provider.dart'; // 引入 userProvider
import '../../providers/shop_provider.dart';

class SelectGPage extends ConsumerStatefulWidget {
  final petShop shop; // 接收商店物件

  const SelectGPage({super.key, required this.shop});

  @override
  _SelectGPageState createState() => _SelectGPageState();
}

class _SelectGPageState extends ConsumerState<SelectGPage> {
          ...
          child: ElevatedButton(
            onPressed: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => SelectGroomingPage(
                    shop: widget.shop,
                  ),
                ),
              );
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color.fromRGBO(255, 239, 239, 1.0), // 按钮背景颜色
            ),
            child: const Text('預定美容服務'),
          ),
        ),
      ),
    );
