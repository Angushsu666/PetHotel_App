...
class SortPetGrooming extends StatefulWidget {
  final List<petShop> shops;

  const SortPetGrooming({super.key, required this.shops});

  @override
  _SortPetGroomingState createState() => _SortPetGroomingState();
}

class _SortPetGroomingState extends State<SortPetGrooming> {
                ...
                String roomsInfo = shop.roomCollection.entries
                    .map((entry) {
                      String typesInfo = entry.value.entries
                          ...
                          .map((typeEntry) =>
                              '${typeEntry.key}: ${typeEntry.value}元')
                          .join(', ');
                ...
                trailingButton = ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => SelectGPage(shop: shop)),
                    );
                  },
                  style: ButtonStyle(
